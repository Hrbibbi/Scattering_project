...
def pCN_explore(GP_factorization, domain, planewaves, measurement_values,
                stepsize=0.5, beta=10, num_iter=20000, warmup_period=0.5,top_index=10):
    '''
    Layered pCN exploration for proposal generation.

    Returns:
        proposals_blocks: shape (num_blocks, 10, ...) array of best proposals
        likelihoods_blocks: shape (num_blocks, 10) array of their log-likelihoods
    '''
    ...
    block_size = 1000
    #num_blocks = int(num_iter * (1 - warmup_period) // block_size)
    proposals_blocks = []
    likelihoods_blocks = []
    ...
    for k in range(num_iter):
        ...
        # After warm-up, store accepted proposals for ranking
        if k >= int(num_iter * warmup_period):
            block_proposals.append(proposal.copy())
            block_likelihoods.append(L_old)

            # At the end of each block
            if len(block_proposals) == block_size:
                # Select top 10 proposals from this block
                top_indices = np.argsort(block_likelihoods)[-top_index:][::-1]  # best first
                top_proposals = [block_proposals[i] for i in top_indices]
                top_likelihoods = [block_likelihoods[i] for i in top_indices]

                proposals_blocks.append(top_proposals)
                likelihoods_blocks.append(top_likelihoods)

                # Reset block
                block_proposals = []
                block_likelihoods = []

    return np.array(proposals_blocks), np.array(likelihoods_blocks)
    ...
    for k in range(num_iter):
        ...
        if uk < Ak:
            ...
            if k>num_iter*warmup_period:
                ...
                accepted_likelihood.append(L_new/beta)
...
def MC_pCN(
    ...
    top_n_per_chain=10,top_index_explore=5,true_values=None
):
    # -------------------------------------------
    # Phase 1: Broad exploration
    # -------------------------------------------
    print("Phase 1: Exploration")
    proposal_blocks,likelihoods_blocks= pCN_explore(GP_factorization,domain,planewaves,measurement_values,
                                                    stepsize=delta_explore,beta=beta_explore,num_iter=num_iter_explore,top_index=5)
    starts_refine1 = proposal_blocks.reshape(-1, *proposal_blocks.shape[2:])
    n_phase2,_,_=np.shape(starts_refine1)
    all_likelihoods = likelihoods_blocks.flatten()
...
if True:
    ...
    def run_clean_pCN_analysis(runname="test"):
        ...
        X, Y, Z = Domains.cylinder_cartesian_grid(1, 1, numpoints_xy=4, numpoints_z=4)
        N = X.shape[0]
        Dom = Domains.Domain(X, Y, Z)
        #print(np.shape(Dom))
        #setup alpha_tensor
        R=np.sqrt(X**2+Y**2)#np.linspace(0,np.pi,N) 
        yaw=0*np.pi/2*np.ones_like(X)#R/R
        pitch=0*np.pi/4*np.ones_like(X)          #np.pi/4*np.ones_like(X)
        roll=0*np.pi/4 *np.ones_like(X) #Z
        ...
        if True:
            ...
            X_sample,Y_sample,Z_sample=X_sample[::1],Y_sample[::1],Z_sample[::1]
            ...
            final_proposals,final_loglikes=MC_pCN(GP_factor,Dom,PW1,true_measurements,
                                num_iter_explore=1000,num_iter_refine=1000,beta_explore=1,beta_refine=100,top_index_explore=10,
...
#run_clean_pCN_analysis(runname="test_test")
...
if True:
 # SETUP: Test object (unit quaternions)
    ...
    pitch=np.pi/4*np.ones_like(X)          #np.pi/4*np.ones_like(X)
    roll=np.pi/4 *np.ones_like(X) #Z
    true_angles=np.column_stack([yaw,pitch,roll])
    alpha_true=Rot.from_euler('ZYX', true_angles, degrees=False).as_matrix()

    final_proposals=np.load("results/test_test/final_proposals.npy")
    final_loglikes =np.load("results/test_test/final_loglikes.npy")
    print(np.shape(final_proposals))
    print(np.shape(final_loglikes))
    best_index=np.argsort(final_loglikes)[-100:][::-1]
    best_proposals=final_proposals[best_index]
    plot_euler_angles(best_proposals,true_angles,Dom,output_folder="results/test_test",filename_prefix="10_best_euler")
    plot_alpha_components(best_proposals,alpha_true,Dom,output_folder="results/test_test",filename_prefix="10_best_alpha")