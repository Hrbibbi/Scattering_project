...
def get_reflected_field_at_points(points,PW,mu,epsilon_substrate,epsilon_air):
    ...
    print(theta_inc)
    print(epsilon_air/epsilon_substrate*np.sin(theta_inc))
    ...
    print(f" Perpendicular coefficent: {r_perp}, Parallel coefficent: {r_par}")
    return E_ref, H_ref
...
def compute_fields_from_csv(param_file, testpoints_file, output_file):
    ...
    PW = Plane_wave(propagation_vector, polarization, epsilon_air, mu, omega)
    E,H=get_reflected_field_at_points(testpoints,PW,mu,epsilon_substrate,epsilon_air)
