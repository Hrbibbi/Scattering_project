...
@njit(parallel=True)
def _jitted_einsum4(amnd, mr):
    """
    Generalized einsum for 'amnd, m r -> a r n d'.
    amnd: shape (A, M, N, D)
    mr:   shape (M, R)
    returns out: shape (A, R, N, D)
      out[a, r, n, d] = sum_m amnd[a, m, n, d] * mr[m, r]
    """
    A, M, N, D = amnd.shape
    R = mr.shape[1]
    out = np.zeros((A, R, N, D), dtype=amnd.dtype)
    # parallelize over a and r
    for a in prange(A):
        for r in range(R):
            for n in range(N):
                for d in range(D):
...
def compute_scattered_field_at_point_jit(points, int_coeff, InteriorDipoles):
    ...
    total_1 = _jitted_einsum4(evals1, C_1)  # (2, R, N, 3)
    total_2 = _jitted_einsum4(evals2, C_2)  # (2, R, N, 3)
